...
def getPayloadData():
        ...
        if (int(PC1) > 8):
            data = pd.read_csv('Flight_5063.csv', header=None, names=["TEAM_ID", "MISSION_TIME", "PACKET_COUNT", "MODE", "STATE", "ALTITUDE", 
                "HS_DEPLOYED", "PC_DEPLOYED", "MAST_RAISED", "TEMPERATURE", "VOLTAGE", 
                "GPS_TIME", "GPS_ALTITUDE", "GPS_LATITUDE", "GPS_LONGITUDE", "GPS_SATS",
        ...
        return(tPlusP, altP, tempP, voltP, gpsAlt, gpsLat, gpsLong, tiltX, tiltY, accel)
...
def setyAxis():      #only done once in drawchart function to set axies
    ...
    _VARS['pltAxis8'].set_ylabel('Acceleration')
    ...
    _VARS['pltAxis8'].set_title('Acceleration (m/s^2) vs Time(s)')
...
def updatePayloadChart():   #THIS TAKES ALL DATA AND GRAPHS IT
    ...
    _VARS['pltsubFig0'].plot(payTPlus, payAlt, '-b')

    _VARS['pltsubFig1'].plot(payTPlus, payTemp, '-r')

    _VARS['pltsubFig2'].plot(payTPlus, payVolt, '-y')

    _VARS['pltsubFig3'].plot(payTPlus, tiltX, 'orange')

    _VARS['pltsubFig4'].plot(payTPlus, tiltY, 'orange')

    _VARS['pltsubFig5'].plot(payTPlus, gpsAlt, '-g')

    _VARS['pltsubFig6'].plot(payTPlus, gpsLat, '-g')

    _VARS['pltsubFig7'].plot(payTPlus, gpsLong, '-g')

    _VARS['pltsubFig8'].plot(payTPlus, accel, 'purple')
...
while True:
    ...
    data = pd.read_csv('Flight_5063.csv', usecols = ["PACKET_COUNT"])       #checks to see if line has been added to canister csv file
